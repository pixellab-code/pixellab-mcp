...
describe('MCP Tool: get_balance', () => {
  ...
  it('should retrieve account balance', async () => {
    ...
    expect(textContent?.text).toContain('PixelLab account balance');
    expect(textContent?.text).toContain('USD');
    expect(textContent?.text).toMatch(/\$\d+\.\d{2}/); // Should contain dollar amount
  }, 30000);
  ...
  it('should handle multiple balance checks', async () => {
    ...
    expect(textContent1?.text).toContain('PixelLab account balance');
    expect(textContent2?.text).toContain('PixelLab account balance');

    // Balance should be consistent (or slightly lower due to usage)
    const balance1Match = textContent1?.text.match(/\$(\d+\.\d{2})/);
    const balance2Match = textContent2?.text.match(/\$(\d+\.\d{2})/);
